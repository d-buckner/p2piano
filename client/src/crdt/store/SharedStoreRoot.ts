...
import { CRDTDocument } from '../services/CRDTDocument';
...
export class SharedStoreRoot {
  ...
  async initialize(realTimeController: RealTimeController) {
    ...
    this.document.updateActorId(userId);
  ...
  change<K extends SharedStoreKey>(
    ...
    this.document.change(storeKey, changeFn);
  ...
  private handleSyncMessage(peerId: string, message: Uint8Array): void {
    ...
    const [newSyncState, hasChanges, patches] = this.document.receiveSyncMessage(syncState, message);
  ...
  private sendSyncToPeer(peerId: string): void {
    ...
    const [newSyncState, syncMessage] = this.document.generateSyncMessage(syncState);
  ...
  private initiateSyncWithConnectedPeers(): void {
    ...
    const myActorId = this.document.getActorId();
   ...
   */
  getActorId(): string {
    return this.document.getActorId();
  }
