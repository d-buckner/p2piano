...
export const selectSelectedMidiInput = (state: RootState) => {
  const { selectedInputId, inputs } = selectMidi(state);
  return selectedInputId ? inputs[selectedInputId] || null : null;
};
